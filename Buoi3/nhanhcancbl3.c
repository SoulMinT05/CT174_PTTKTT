...
void nhanhCan(int i, int n, int x[], float *V, float *CT, float *TGT, float *GLNTT, DoVat *dsdv){
    int j;
    int vk = min(1, *V/dsdv[i].TL);
    for(j=vk; j>=0; j--){
        *TGT += j*dsdv[i].GT;
        *V -= j*dsdv[i].TL;
        *CT = *TGT + *V * dsdv[i+1].DG;
        if(*CT > *GLNTT){
            ...
            if((i==n-1) || (*V==0))
                ...
                nhanhCan(i+1, n, x, V, CT, TGT, GLNTT, dsdv);
        }
        x[i]=0;
        *TGT -= j*dsdv[i].GT;
        *V += j*dsdv[i].TL;
...
int main(){
    ...
    nhanhCan(0, n, x, &V, &CT, &TGT, &GLNTT, dsdv);
    inDSDV(dsdv, n, W);
}